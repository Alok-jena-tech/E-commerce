...
// Async thunk to get a checkout session
export const getCheckout=createAsyncThunk("checkout/getCheckout",async(id, {rejectWithValue})=>{
    try {
        const response=await axios.get(`${import.meta.env.VITE_BACKEND_URL}/api/checkout/${id}`,{
            headers:{
                Authorization:`Bearer ${localStorage.getItem("userToken")}`,
            },
        });
